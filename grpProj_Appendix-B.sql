...
#########################################################
SELECT SUM(population) AS total_population_in_asia FROM (SELECT MAX(CAST(IFNULL(NULLIF(population, ''), 0.0) AS DECIMAL(11,1))) AS population, location FROM covid19data WHERE continent = 'Asia' GROUP BY location) AS subquery;
